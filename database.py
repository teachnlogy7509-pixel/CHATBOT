...
def setup_database():
    ...
    connection.execute("""CREATE TABLE IF NOT EXISTS pdfs (
...
def save_user(user):
    connection = database()
    connection.execute("""INSERT INTO users (user_id,name,username) VALUES (?,?,?)
...
def update_score(user, points, was_correct):
    save_user(user)
    connection = database()
    connection.execute("""UPDATE users SET total_score=total_score+?, attempted=attempted+1,
        correct=correct+? WHERE user_id=?""",
        (points, 1 if was_correct else 0, user.id))
    connection.commit(); connection.close()

def save_poll(poll_id, chat_id, creator_id, question, correct_option, explanation):
    connection = database()
    connection.execute("""INSERT OR REPLACE INTO polls
        (poll_id,chat_id,creator_id,question,correct_option,explanation,created_at)
        VALUES (?,?,?,?,?,?,?)""",
        (poll_id, chat_id, creator_id, question, correct_option, explanation,
...
def save_pdf(file_id, file_name, uploaded_by):
    connection = database()
    connection.execute("""INSERT INTO pdfs (file_id,file_name,uploaded_by,uploaded_at)
        VALUES (?,?,?,?)""",
...
def get_pdfs():
    connection = database()
    rows = connection.execute("SELECT * FROM pdfs ORDER BY id DESC").fetchall()
    connection.close()
    return rows

def save_schedule(chat_id, admin_id, topic, count, hour, minute, timer):
    connection = database()
    cur = connection.execute("""INSERT INTO schedules
        (chat_id,admin_id,topic,question_count,hour,minute,timer_seconds,created_at)
        VALUES (?,?,?,?,?,?,?,?)""",
        (chat_id,admin_id,topic,count,hour,minute,timer,datetime.now(timezone.utc).isoformat()))
    connection.commit()
...
def delete_schedules(chat_id, admin_id):
    connection = database()
    connection.execute("DELETE FROM schedules WHERE chat_id=? AND admin_id=?",
                       (chat_id,admin_id))
    connection.commit(); connection.close()
