...
async def pdf_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Only the configured admin can upload PDFs."""
    ...
    await message.reply_text(
        f"✅ PDF सेव हो गई!\n\n📄 {file_name}\n"
        "अब यूज़र /pdfs कमांड से इसे पा सकते हैं।"
    )
...
async def pdfs_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📚 उपलब्ध PDFs भेज रहा हूँ...")
    ...
    for row in rows:
        try:
            ...
            print(f"PDF send error ({row['file_name']}): {e}")
...
def main():
    ...
    app.add_handler(
        MessageHandler(filters.Document.PDF, pdf_upload)
    )
